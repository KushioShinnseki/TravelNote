...
function unique(values) {
  return Array.from(new Set((Array.isArray(values) ? values : []).map(value => text(value)).filter(Boolean)));
}
...
function decodePacket(raw) {
  ...
  return {
    ...
    accountId: text(packet.accountId),
    profile: { home: text(packet.profile && packet.profile.home) },
    tagCatalog: unique(packet.tagCatalog),
    destinations: packet.destinations.map((item, index) => ({
      id: text(item.id, `d-${index}`),
      name: text(item.name),
...
function syncRecords(previous, incoming) {
  const oldById = {};
  (previous || []).forEach(item => { oldById[item.id] = item; });
  const incomingIds = {};
  let created = 0;
  let updated = 0;
  const items = incoming.map(item => {
    incomingIds[item.id] = true;
    if (!oldById[item.id]) created += 1;
    else if (!equal(oldById[item.id], item)) updated += 1;
    return item;
  });
